...
> **Target Aplikasi:** Demo Website & Management Console Vije Boutique Resort ([hotel.hasanarofid.site](https://hotel.hasanarofid.site))  
...
Untuk mengakses panel kontrol operasional manajemen hotel (_Admin Hospitality Console_), peramban (_browser_) dapat diarahkan ke URL login resmi:
...
1. Buka peramban (_Google Chrome, Mozilla Firefox, atau Safari_).
...
Website publik Vije Boutique Resort memfasilitasi tamu untuk melakukan pemesanan langsung (_Direct Booking Channel_) tanpa komisi OTA.
...
2. **Formulir Floating Booking Bar (`#booking`):**
    ...
    - **Pilih Kamar / Villa:** Pilih kategori akomodasi yang diinginkan (_Grand Ocean Pool Villa, Sanctuary Garden Suite, Royal Residence_). Sistem menampilkan estimasi total tarif secara real-time.
3. **Pengisian Data Tamu:**
    ...
    - **Permintaan Khusus (Opsional):** Catatan tambahan (misal: _Honeymoon setup, airport transfer_).
...
Manajemen hotel atau General Manager dapat menambah, mengedit, atau meng-update harga kamar sesuai musim (_High Season / Low Season_) melalui menu **Rooms & Villas (`/admin/rooms`)**.

### 4.1 Mengubah Harga & Kuota Kamar

1. Buka menu **Rooms & Villas** (`/admin/rooms`).
2. Cari kartu kamar yang ingin disesuaikan (misal: _Grand Ocean Pool Villa_).
...
5. Klik **"Simpan Perubahan"**.  
   _Tarif baru akan langsung berlaku secara real-time pada formulir reservasi publik._
...
- **Jawaban:** Staf yang sedang login dapat mengklik nama profil di pojok kanan atas, pilih **Profile Settings**, lalu masukkan password lama dan password baru pada bagian _Update Password_.
...
### Q3: Apakah dua tamu bisa memesan kamar yang sama di tanggal yang sama (_Double Booking_)?

- **Jawaban:** **Tidak bisa.** Sistem backend dilengkapi dengan _Pessimistic Database Locking_ (`lockForUpdate()`). Saat satu pemesanan sedang diproses pada tanggal tersebut, kuota unit langsung dikunci secara atomic di server.
